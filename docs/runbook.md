...
```bash
./scripts/run_scenario.sh baseline   # baseline — raw failure propagates
./scripts/run_scenario.sh retry      # +retry+idempotency
./scripts/run_scenario.sh failfast   # +deadline+bulkhead+CB (slow B, C=80)
...
```bash
./scripts/run_scenario.sh baseline   # required for verify_retry.sh and verify_failfast.sh
./scripts/run_scenario.sh retry      # required for verify_retry.sh
./scripts/run_scenario.sh failfast   # required for verify_failfast.sh
./scripts/run_scenario.sh selfheal   # required for verify_selfheal.sh
```
