# Developer Prompt — T16: run_scenario.sh restructure

## Context
GitHub issue #18. Read .clinerules/ before starting. Read tmp/T16/plan.md.
DEPENDS ON #17 (T15) being merged (values-scenario{1,2,3,4}.yaml must exist).

## Task
Rewrite scripts/run_scenario.sh with new interface: ./scripts/run_scenario.sh <1|2|3|4>
...
## DoD Proof Commands (save to tmp/T16/proof.txt)
```bash
# Proof 1: Syntax check
bash -n scripts/run_scenario.sh
echo "Syntax OK: $?"

# Proof 2: Usage help shown
./scripts/run_scenario.sh 2>&1 | grep "Usage"
# Expected: Usage: ... <1|2|3|4>

# Proof 3: Invalid arg rejected
./scripts/run_scenario.sh 99 2>&1 | grep "Error"
# Expected: Error: argument must be 1, 2, 3, or 4
```

## Commit Message (save to tmp/T16/commit_msg.txt)
```
feat: rewrite run_scenario.sh with 4-scenario interface

- New interface: ./scripts/run_scenario.sh <1|2|3|4>
- Composes values-common.yaml + values-scenario<N>.yaml
- Artifacts to tmp/artifacts/scenario<N>/
- Scenario 3 uses C=80 for overload; Scenario 4 injects TCP fault at t=15s
- Removes old <scenario> <mode> interface
