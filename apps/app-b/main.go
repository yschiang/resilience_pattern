...
func (s *server) Work(ctx context.Context, req *pb.WorkRequest) (*pb.WorkReply, error) {
	...
	//
	// DEMO LIMITATION: sync.Map is local to each pod (not shared across 3 B pods).
	// With random load balancing, only ~33% of retries hit the same pod → cache.
	// Production requires shared state:
	//   - Redis/Memcached: Shared cache with TTL (Stripe, Slack pattern)
	//   - Database: Unique constraint on (user_id, idempotency_key) table
	//   - Session affinity: Pin client → same backend pod (reduces resilience)
	// This demo shows the PATTERN (check cache → process → store), not the
	// production architecture. Focus on learning WHY idempotency matters.
	if req.GetId() != "" {
